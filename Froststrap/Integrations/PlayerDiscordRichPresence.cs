...
namespace Froststrap.Integrations
{
    public class PlayerDiscordRichPresence : IDisposable
    {
        ...
        public PlayerDiscordRichPresence(ActivityWatcher activityWatcher)
        {
            ...
            _activityWatcher.OnGameJoin += (_, _) => SafeTask.Run(
                () => SetCurrentGame(), ErrorSeverity.NonFatal, "PlayerDiscordRichPresence::OnGameJoin");
            _activityWatcher.OnGameLeave += (_, _) => SafeTask.Run(
                () => SetCurrentGame(), ErrorSeverity.NonFatal, "PlayerDiscordRichPresence::OnGameLeave");
        ...
        private void ProcessSetRichPresence(Message message, bool implicitUpdate)
        {
            ...
            if (_smallImgBeingFetched != null || _largeImgBeingFetched != null)
            {
                ...
                SafeTask.Run(() => UpdatePresenceIconsAsync(_smallImgBeingFetched, _largeImgBeingFetched, implicitUpdate, _fetchThumbnailsToken.Token), ErrorSeverity.NonFatal, "PlayerDiscordRichPresence::UpdatePresenceIconsAsync");
            }
        }
