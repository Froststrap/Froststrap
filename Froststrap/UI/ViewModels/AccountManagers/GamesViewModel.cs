	...
	public partial class GamesViewModel : ObservableObject
	{
		...
		private async Task FindAndJoinServer(long placeId, AltAccount account)
		{
			...
			try
			{
				...
				if (datacentersResult == null)
				{
					_ = Frontend.ShowMessageBox("Failed to load server regions. Please try again later.", MessageBoxImage.Error);
					return;
				}
				...
				{
					_ = Frontend.ShowMessageBox("Cannot find a cookie to use, Please log in to account manager.", MessageBoxImage.Error);
					return;
				}
				...
				{
					_ = Frontend.ShowMessageBox("Please select a region in Region Selector first.", MessageBoxImage.Warning);
					return;
				}
				...
				while (attemptCount < maxAttempts)
				{
					...
					{
						_ = Frontend.ShowMessageBox($"Could not find a server in {selectedRegion} after searching all available servers.", MessageBoxImage.Information);
						return;
					}
				...
				_ = Frontend.ShowMessageBox($"Could not find a server in {selectedRegion} after {maxAttempts} pages. Try selecting a different region.", MessageBoxImage.Information);
			}
			finally
			{
		...
		private async Task AutoFindAndJoinGameAsync(long placeId)
		{
			...
			if (placeId == 0)
			{
				_ = Frontend.ShowMessageBox("Invalid Place ID.", MessageBoxImage.Warning);
				return;
			}
			...
			catch (Exception ex)
			{
				...
				_ = Frontend.ShowMessageBox($"Failed to find and join server: {ex.Message}", MessageBoxImage.Error);
			}
			finally
			{
		...
		private async Task AutoFindAndJoinSelectedGameAsync()
		{
			...
			if (SelectedSearchResult == null)
			{
				...
				{
					_ = Frontend.ShowMessageBox("Please select a game first or enter a valid Place ID.", MessageBoxImage.Warning);
					return;
				}
		...
		private async Task LaunchRoblox()
		{
			...
			{
				_ = Frontend.ShowMessageBox("Please enter a Place ID.", MessageBoxImage.Warning);
				return;
			}

			if (!long.TryParse(PlaceId, out long placeId))
			{
				_ = Frontend.ShowMessageBox("Please enter a valid Place ID.", MessageBoxImage.Warning);
				return;
			}
		...
		private async Task LaunchSubplace(long placeId)
		{
			...
			catch (Exception ex)
			{
				...
				_ = Frontend.ShowMessageBox($"Failed to launch subplace: {ex.Message}", MessageBoxImage.Error);
			}
		}
		...
		private async Task JoinPrivateServer(string accessCode)
		{
			...
			try
			{
				...
				if (!long.TryParse(PlaceId, out long placeId) || placeId == 0)
				{
					_ = Frontend.ShowMessageBox("Please select a game first (set Place ID).", MessageBoxImage.Warning);
					return;
				}
			...
			catch (Exception ex)
			{
				...
				_ = Frontend.ShowMessageBox($"Failed to join server: {ex.Message}", MessageBoxImage.Error);
			}
			finally
			{
				Dispatcher.UIThread.Invoke(() => IsPrivateServersModalOpen = false);
