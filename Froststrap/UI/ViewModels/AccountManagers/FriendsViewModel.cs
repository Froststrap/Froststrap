		...
		private async Task JoinFriend(FriendInfo friend)
		{
			...
			try
			{
				...
				if (friend == null)
				{
					...
					_ = Frontend.ShowMessageBox("Invalid friend information.", MessageBoxImage.Warning);
					return;
				}
				...
				{
					_ = Frontend.ShowMessageBox($"{friend.DisplayName} is not currently in a game.", MessageBoxImage.Information);
					return;
				}
				...
				{
					_ = Frontend.ShowMessageBox($"Unable to get game information for {friend.DisplayName}.", MessageBoxImage.Warning);
					return;
				}
				...
				{
					_ = Frontend.ShowMessageBox($"Unable to determine the game {friend.DisplayName} is playing.", MessageBoxImage.Warning);
					return;
				}
				...
				{
					_ = Frontend.ShowMessageBox($"Unable to get server information for {friend.DisplayName}'s game.", MessageBoxImage.Warning);
					return;
				}
				...
				catch (Exception ex)
				{
					...
					_ = Frontend.ShowMessageBox($"Failed to join {friend.DisplayName}'s game: {ex.Message}", MessageBoxImage.Error);
				}
			}
			catch (Exception ex)
			{
				App.Logger.WriteLine(LOG_IDENT_JOIN_FRIEND, $"Exception: {ex.Message}");
				_ = Frontend.ShowMessageBox($"Failed to join {friend?.DisplayName ?? "friend"}: {ex.Message}", MessageBoxImage.Error);
			}
		}
