        ...
        public async Task DoInstall()
        {
            ...
            if (!IsImplicitInstall)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    _ = Frontend.ShowMessageBox(Strings.Installer_Install_CannotOverwrite, MessageBoxImage.Error);
        ...
        public static async void HandleUpgrade()
        {
            ...
            if (existingVer is not null)
            {
                ...
                if (Utilities.CompareVersions(existingVer, "1.5.1.0") == VersionComparison.LessThan)
                {
                    ...
                    {
                        string[] foldersToMove = Directory.GetDirectories(legacyRoot);

                        foreach (var sourceFolderPath in foldersToMove)
                        {
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        public async void ImportSettingsFromSelectedApp()
        {
            ...
            {
                _ = Frontend.ShowMessageBox(Strings.Installer_InstallationNotFound, MessageBoxImage.Information);
                return;
            }
